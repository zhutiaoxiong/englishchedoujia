...
public class OCtrlRegLogin {
    ...
    public void processResult(int protocol, JsonObject obj,String CACHE_ERROR) {
        switch (protocol) {
            ...
            case 1126:
                ...
                break;
            case 1603:
                backdata_1603_getVerficodeByEmail(obj,CACHE_ERROR);
                break;
            case 1604:
                backdata_1604_reg(obj,CACHE_ERROR);
                break;
        }
    }
    // ============================ccmd==================================
    ...
    public void ccmd1603_getVerificationCodeByEmaol(String email) {
        JsonObject data = new JsonObject();
        data.addProperty("email", email);
        HttpConn.getInstance().sendMessage(data, 1603);
    }

    /**
     * 邮箱注册信息提交
     **/
    public void ccmd1604_Reg(String email,String phoneNum, String password, String code) {
        JsonObject data = new JsonObject();
        data.addProperty("email", email);
        data.addProperty("phoneNum", phoneNum);
        data.addProperty("password", MD5.MD5generator(password));
        data.addProperty("code", code);
        HttpConn.getInstance().sendMessage(data, 1604);
    }
    /**
     * 拿用户信息
     **/
...
//            BlueLinkNetSwitch.saveSwitchShake(shakeInfo);
     ...
     * 获取邮箱验证码
     */
    public void backdata_1603_getVerficodeByEmail(JsonObject obj,String CACHE_ERROR) {
        if (CACHE_ERROR.equals("")) {
            ODispatcher.dispatchEvent(OEventName.VERIFICATION_CODE_EMAIL_BACKOK);
            ODispatcher.dispatchEvent(OEventName.GLOBAL_POP_TOAST,"验证码已发送到邮箱，请注意查收！");
        }
    }
    /**
     * 注册信息提交
     **/
    public void backdata_1604_reg(JsonObject obj,String CACHE_ERROR) {
        if (CACHE_ERROR.equals("")) {
            JsonObject userInfo = obj.getAsJsonObject("userInfo");
            String token = OJsonGet.getString(obj, "token");
            ManagerLoginReg.getInstance().saveUserInfo(userInfo);

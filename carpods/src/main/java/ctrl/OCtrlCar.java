...
public class OCtrlCar {
    ...
    public void processResult(int protocol, JsonObject obj, String CACHE_ERROR) {
        switch (protocol) {
            ...
            case 6001:
                ...
                break;
            case 11001:
                backdata_11001_zhendongbaojing_set(obj,CACHE_ERROR);
                break;
            case 11002:
                backdata_11002_zhendongbaojing_info(obj,CACHE_ERROR);
                break;
        }
    }
    // ============================error==================================
    ...
    public void ccmd1203_getcarlist() {
        ...
        if (BuildConfig.DEBUG)Log.e("拿他妈的车列表", "发协议拿");
    }
...
private boolean isRegetRemoteControl;
    ...
    public void ccmd11001_zhendongbaojing(String terMinalNum ,int type,int instructType,int isOpen,int extra) {
        JsonObject data = new JsonObject();
        data.addProperty("terminalNum",terMinalNum);
        data.addProperty("type",type);
        data.addProperty("instructType",instructType);
        data.addProperty("isOpen",isOpen);
        data.addProperty("extra",extra);
        HttpConn.getInstance().sendMessage(data, 11001);
        if (BuildConfig.DEBUG) Log.e("110001", "carId"+11001);
    }
    public void ccmd11002_zhendongbaojing(String terMinalNum) {
        JsonObject data = new JsonObject();
        data.addProperty("terminalNum",terMinalNum);
        HttpConn.getInstance().sendMessage(data, 11002);
        if (BuildConfig.DEBUG) Log.e("110001", "carId"+11002);
    }
...
//            ManagerRemoteControl.getInstance().saveBeanRemoteControl(productRemote);
     ...
     * 2108 舒适进入
     **/
    private void backdata_11001_zhendongbaojing_set(JsonObject obj, String CACHE_ERROR) {
        if (CACHE_ERROR.equals("")) {
            ODispatcher.dispatchEvent(OEventName.MINIX_VIBRATION_SET_BACK);
        }
    }
    /**
     * 6001结果
     **/
    private void backdata_11002_zhendongbaojing_info(JsonObject obj, String CACHE_ERROR) {
        if (CACHE_ERROR.equals("")) {
            JsonObject productRemote = OJsonGet.getJsonObject(obj, "productSet");
            ManagerVibrationMiniX.getInstance().saveVibrationInfo(productRemote);
            ODispatcher.dispatchEvent(OEventName.MINIX_VIBRATION_BACK);
        }
    }
}

...
import static com.client.proj.carpods.R.id.beizhu_txt;
import static com.client.proj.carpods.R.id.car_num;
import static com.client.proj.carpods.R.id.first_time;
import static com.client.proj.carpods.R.id.location_details;
import static com.client.proj.carpods.R.id.millage;
import static com.client.proj.carpods.R.id.second_time;
import static com.client.proj.carpods.R.id.timelength;


public class AdapterShowGpsPath extends BaseAdapter {
    ...
    public AdapterShowGpsPath(Context context,int rightWidth, List<DataGpsPath> listPath) {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            holder.pathdelete.setTag(holder);
        } else {
            holder = (ViewHolder) convertView.getTag();
        }
//        holder.pos = position;
//        holder.totalPos = list.size();
        LinearLayout.LayoutParams lp1 = new LinearLayout.LayoutParams(MATCH_PARENT,MATCH_PARENT);//ODipToPx.dipToPx(mContext,205)
        holder.item_left.setLayoutParams(lp1);
//        LinearLayout.LayoutParams lp2 = new LinearLayout.LayoutParams(mRightWidth, MATCH_PARENT);
//        holder.item_right.setLayoutParams(lp2);
        if (list == null || list.size() == 0) return null;
        if (list.get(position).comment == null || list.get(position).comment.length() == 0) {
            AbsListView.LayoutParams params = new AbsListView.LayoutParams(MATCH_PARENT, ODipToPx.dipToPx(mContext, 173.5f));
            convertView.setLayoutParams(params);
        } else {
            AbsListView.LayoutParams params = new AbsListView.LayoutParams(MATCH_PARENT, ODipToPx.dipToPx(mContext, 205.5f));
            convertView.setLayoutParams(params);
        }
        if (list.get(position).comment == null || list.get(position).comment.length() == 0) {
        ...
        holder.millage.setText(ResourceUtil.getResourceStr(mContext,R.string.page_xingshi_jilu_licheng) + df.format(info.miles) + ResourceUtil.getResourceStr(mContext,R.string.page_xingshi_jilu_gongli));
        holder.timelength.setText(ResourceUtil.getResourceStr(mContext,R.string.page_xingshi_jilu_shichang)+ info.intervalTime);
...
//            执行ontouch事件
//            itemtouch(holder, convertView,position);
        }
        return convertView;
    }
    /**
     ...
     * @param
     */
//    public void itemtouch(ViewHolder holder,  View convertview,final int position) {
//        holder.movelayout.setOnTouchListener(new View.OnTouchListener() {
//            @Override
...
//                        int buttonX = (int) holder.movelayout.getX();
//                        int moveXL;
...
//                            CarPath.position=R.layout.view_app_gps_path_list;
//                            ManagerGps.path=DataGpsPath.getLatLngs(list.get(position).latlngs);
//                            ManagerGps.startLocation   =  list.get(position).startLocation;
//                            ODispatcher.dispatchEvent(OEventName.ACTIVITY_KULALA_GOTOVIEW, R.layout.view_car_path);
//                        }
//                        if ((upTime - downTime) > 200 && (Math.abs(moveX) < 3|| Math.abs(moveY) < 3)) {
//                            ManagerGps.path=DataGpsPath.getLatLngs(list.get(position).latlngs);
//                            ManagerGps.startLocation   =  list.get(position).startLocation;
...
//    }
    ...
    public void findId1(ViewHolder holder, View convertView) {
        ...
        holder.pathdelete = (ImageView) convertView.findViewById(R.id.pathdelete);
    }
    ...
    public void click(final ViewHolder holder, final int position) {
        holder.pathshare.setOnClickListener(new OnClickListenerMy() {
            ...
            public void onClickNoFast(View v) {
                ...
                OToastSharePath.getInstance().show(holder1.pathshare, "sharepath004", new OToastSharePath.OnClickButtonListener() {
                    @Override
        ...
        holder.pathdelete.setOnClickListener(new OnClickListenerMy() {
            @Override
            public void onClickNoFast(View v) {
                new ToastConfirmNormal(GlobalContext.getCurrentActivity(), null,false)
                        .withTitle(ResourceUtil.getResourceStr(mContext,R.string.page_xingshi_shifoushanchu))
                        .withButton(ResourceUtil.getResourceStr(mContext,R.string.cancle),ResourceUtil.getResourceStr(mContext,R.string.confirm))
        ...
        public void handleMessage(Message msg) {
            if (msg.what == 110) {
                if (list.get(newPosition).isCollect == 0) {
                    ...
                    ODispatcher.dispatchEvent(OEventName.GLOBAL_POP_TOAST,ResourceUtil.getResourceStr(mContext,R.string.page_xingshi_shoucangchenggong));
                    list.get(newPosition).isCollect =1;
                } else {
                    shoucang.setImageResource(R.drawable.pathshoucang);
                    ODispatcher.dispatchEvent(OEventName.GLOBAL_POP_TOAST,ResourceUtil.getResourceStr(mContext,R.string.page_xingshi_shoucangquxiao));

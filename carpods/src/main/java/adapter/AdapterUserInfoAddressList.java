...
public class AdapterUserInfoAddressList extends BaseAdapter{
	private LayoutInflater    mInflater;
	private Context           mContext;
	private List<DataAddress> list;
	private int               currentPosition;
	public AdapterUserInfoAddressList(Context context, List<DataAddress> list) {
		this.mContext = context;
		this.mInflater = LayoutInflater.from(mContext);
		this.list = list;
	}
	public DataAddress getCurrentItem() {
	...
	@Override
	public DataAddress getItem(int position) {
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		// 设初始显示数据
		DataAddress info = list.get(position);
		...
		holder.txt_info.setText(info.province);
		holder.img_splitline.setVisibility(View.INVISIBLE);
		return convertView;
	}
	// ===================================================
	public List<DataAddress> getDataList() {
		return list;
	}

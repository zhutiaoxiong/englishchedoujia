            ...
            public void onClick(View v) {
                if(data.isMyCar==1){
                    new ToastConfirmNormal(GlobalContext.getCurrentActivity(), null,false)
                            .withTitle(ResourceUtil.getResourceStr(getContext(),R.string.termination_of_authorization))
                            .withInfo(ResourceUtil.getResourceStr(getContext(),R.string.determine_the_transport_no_longer_cancel_authorized_vehicles))
...
//                                    long carId= ManagerCarList.getInstance().getCurrentCarID();
                    ...
                    ODispatcher.dispatchEvent(OEventName.GLOBAL_POP_TOAST,ResourceUtil.getResourceStr(getContext(),R.string.page_car_detail_fuchezhu_no));
                }
            }
        });
    }
    ...
    public void invalidateUI() {
        if(data == null){
            ODispatcher.dispatchEvent(OEventName.GLOBAL_POP_TOAST,ResourceUtil.getResourceStr(getContext(),R.string.page_car_detail_no_data));
            return;
        }
        ...
        if(data.isMyCar == 0){//是副车主
            ...
            if(data.isDue == 0){
                txt_bao_info.setText(ResourceUtil.getResourceStr(getContext(),R.string.page_car_detail_meijihuo));
                img_bao.setImageResource(R.drawable.carman_bao_unactive);
                txt_bao_type.setText(ResourceUtil.getResourceStr(getContext(),R.string.page_car_not_jihuo));
                txt_bao_type.setTextColor(Color.parseColor("#6d6d6d"));
            }else if(data.isDue == 1){
                txt_bao_info.setText(ResourceUtil.getResourceStr(getContext(),R.string.page_car_detail_yu)+ ODateTime.time2StringOnlyDate(data.activeTime)+ResourceUtil.getResourceStr(getContext(),R.string.page_car_detail_jihuo));
                img_bao.setImageResource(R.drawable.carman_bao_in);
                txt_bao_type.setText(ResourceUtil.getResourceStr(getContext(),R.string.page_car_detail_baoxiuzhong));
                txt_bao_type.setTextColor(Color.parseColor("#ff0677da"));
            }else if(data.isDue == 2){
                txt_bao_info.setText(ResourceUtil.getResourceStr(getContext(),R.string.page_car_detail_yu)+ ODateTime.time2StringOnlyDate(data.activeTime)+ResourceUtil.getResourceStr(getContext(),R.string.page_car_detail_jihuo));
                img_bao.setImageResource(R.drawable.carman_bao_in);
                txt_bao_type.setText(ResourceUtil.getResourceStr(getContext(),R.string.page_car_detail_yiguobao));
                txt_bao_type.setTextColor(Color.parseColor("#ff0677da"));
            }
        }
    }

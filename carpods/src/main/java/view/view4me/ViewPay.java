        ...
        btn_confirm.setOnClickListener(new OnClickListenerMy() {
            ...
            public void onClickNoFast(View v) {
                ...
                if(currentCar==null||currentCar.isActive==0){
                    new ToastTxt(GlobalContext.getCurrentActivity(), null).withInfo(getResources().getString(R.string.page_apply_tip_active)).show();
                    return;
                }
...
//                        ODispatcher.dispatchEvent(OEventName.GLOBAL_POP_TOAST, "支付宝支付失败!");
            ...
            new ToastTxt(GlobalContext.getCurrentActivity(),null).withInfo(getResources().getString(R.string.top_up_success)).show();
            OCtrlCar.getInstance().ccmd1203_getcarlist();
            selectCar = ManagerCarList.getInstance().getCurrentCar();
            handleChangeData();

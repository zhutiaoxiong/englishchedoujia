...
public class ViewNoKey extends RelativeLayoutBase {
    ...
    private TextView txt_back_run_permision_set;
...
//            msp=new SpannableString("1.\t手机蓝牙保持常开;\n\n2.\t发起配对：以便灭屏时自动连蓝牙(如果打开摇一摇时\n\n\t\t已经设置保活则无需再发起配对)");
            msp=new SpannableString(ResourceUtil.getResourceStr(getContext(),R.string.page_noley_small_tips));
            //设置字体前景色
...
//        }
 ...
 if (key.equals("un_lock_panduan")) {
    ...
    protected void initEvents() {
        ...
        txt_nokey_close.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                clickCount++;
                if(clickCount>=5){
                    if(ManagerNokey.getInstance().getSwitchClose()){
                        lin_nokey_close.setVisibility(View.VISIBLE);
                    }
                }
            }
        });
        ...
        peidui.setOnClickListener(new OnClickListenerMy3000() {
            ...
            public void onClickNoFast(View view) {
                ...
                if(deviceList1!=null&&deviceList1.size()>0){
                    for (BluetoothDevice devices : deviceList1) {
                        ...
                        if (!TextUtils.isEmpty(car.bluetoothName) && !TextUtils.isEmpty(devices.getName()) && car.bluetoothName.equals(devices.getName())) {
                            isHaveBond = true;
                            new ToastTxt(GlobalContext.getCurrentActivity(), null, false).withInfo(ResourceUtil.getResourceStr(getContext(),R.string.page_nokey_already_peidui)).quicklyShow();
                           return;
                        }
                    }
                }
                ...
                if (loading_view_layout.getVisibility() == View.INVISIBLE) {
                    ...
                    loading_view_textview.setText(ResourceUtil.getResourceStr(getContext(),R.string.page_nokey_peidui_wait));
...
//                }
                if (!BlueLinkReceiver.getInstance().getIsBlueConnOK()) {
                    new ToastTxt(GlobalContext.getCurrentActivity(), null).withInfo(ResourceUtil.getResourceStr(getContext(),R.string.page_control_no_bloetooth_connect)).show();
                    return;
                }
                isSetKaoJInKai = true;
            ...
            public void onClick(View view) {
                if (closeIsDefault) {
                    ...
                    if ((closeDefaultValue - openDeaufaltValue) <= 8 && closeDefaultValue <= 98) {
                        ...
                        new ToastTxt(GlobalContext.getCurrentActivity(), null).withInfo(ResourceUtil.getResourceStr(getContext(),R.string.page_nokey_already_big)).show();
                    }
                }
                txt_elc_close.setText(closeDefaultValue + "");
                sendData();
            ...
            public void onClick(View view) {
                if (closeIsDefault) {
                    ...
                    if (closeDefaultValue >= 52 && (closeDefaultValue - openDeaufaltValue) >= 4) {
                        ...
                        new ToastTxt(GlobalContext.getCurrentActivity(), null).withInfo(ResourceUtil.getResourceStr(getContext(),R.string.page_nokey_already_small)).show();
                    }
                }
                txt_elc_close.setText(closeDefaultValue + "");
                sendData();
...
//                BlueAdapter.getInstance().sendMessage(dataOpen);//发送指令
    ...
    private void sendDataMy(CacheNokeyInfo cacheNokeyInfo) {
        if (cacheNokeyInfo == null || cacheNokeyInfo.openData == 0 && cacheNokeyInfo.closeData == 0) {
            new ToastTxt(GlobalContext.getCurrentActivity(), null).withInfo(ResourceUtil.getResourceStr(getContext(),R.string.page_nokey_please_shezhi)).show();
            return;
        }

    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch(event.getAction()){
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if(firstLocation==1){
                    if(moveY>downY){
                        int topThumbLocation= ODipToPx.dipToPx(getContext(),61);
//                        int marginTop= ODipToPx.dipToPx(getContext(),3);
                        ...
                        if(downY>=marginTop&&downY<=topThumbLocation){
                            //执行下滑滑动操作
                            int maxDistabce= ODipToPx.dipToPx(getContext(),55);
                    ...
                    if(moveY<downY){
                        int marginBottom= ODipToPx.dipToPx(getContext(),116);
                        int bottomThumbLocation= ODipToPx.dipToPx(getContext(),55);
...
//                        int marginTop= ODipToPx.dipToPx(getContext(),3);
                        int marginTop= 0;
                        if(downY>=marginTop&&downY<=topThumbLocation){
                            int percentFourThumb= ODipToPx.dipToPx(getContext(),61/4);
    ...
    private void setThumbLocationScrollUp(float moveyY,float downYy){
        ...
        layout.setMargins(0, ODipToPx.dipToPx(getContext(),55)-(int)(downYy-moveyY), 0, 0+(int)(downYy-moveyY));
    ...
    private void setThumbLocationScrollDown(float moveYy,float downYy){
        ...
        layout.setMargins(0, 0+(int)(moveYy-downYy), 0, ODipToPx.dipToPx(getContext(),55)-(int)(moveYy-downYy));
        //设置button的新位置属性,left，top，right，bottom
        thumb.setLayoutParams(layout);
    }
        ...
        public void handleMessage( Message msg) {
            if(msg.what==110){
                ...
                if(msg.arg1==1){
                    LayoutParams layout=(LayoutParams)thumb.getLayoutParams();
                    //获得button控件的位置属性，需要注意的是，可以将button换成想变化位置的其它控件
                    layout.setMargins(0, 0, 0, ODipToPx.dipToPx(getContext(),55));
                    //设置button的新位置属性,left，top，right，bottom
                    thumb.setLayoutParams(layout);
                    thumb.invalidate();
                }else{
                    LayoutParams layout=(LayoutParams)thumb.getLayoutParams();
                    //获得button控件的位置属性，需要注意的是，可以将button换成想变化位置的其它控件
                    layout.setMargins(0, ODipToPx.dipToPx(getContext(),55), 0, 0);
                    //设置button的新位置属性,left，top，right，bottom
                    thumb.setLayoutParams(layout);
                    thumb.invalidate();
                }

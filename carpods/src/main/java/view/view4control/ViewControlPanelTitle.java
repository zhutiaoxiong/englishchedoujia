...
import common.blue.ManagerCarStatus;
...
public class ViewControlPanelTitle extends RelativeLayoutBase {
	...
	private  ImageView iv_batterry;
	private  TextView tv_batterry;
	private  TextView tv_batterry_mode;
	...
	public ViewControlPanelTitle(Context context, AttributeSet attrs) {
		...
		iv_batterry= findViewById(R.id.iv_batterry);
		tv_batterry= findViewById(R.id.tv_batterry);
		tv_batterry_mode= findViewById(R.id.tv_batterry_mode);
			...
			public void onClickNoFast(View v) {
				...
				if(  EquipmentManager.isMini()||(EquipmentManager.isMinJiaQiang()&&EquipmentManager.isMinNoMozu())||EquipmentManager.isShouweiSix()){//MINI版本
					if(BlueLinkReceiver.getIsBlueConnOK()){
						...
						new ToastTxt(GlobalContext.getCurrentActivity(),null,false).withInfo(getResources().getString(R.string.page_apply_tip_blue_unconnect)).quicklyShow();
					}
				}else{
		...
		public void handleMessage(@NonNull Message msg) {
			if(msg.what==5555){
				...
				if(isConnect.equals("0")){
					...
					txt_tip.setText(getResources().getString(R.string.control_tip_car_net_false));
				...
				if(!isNetConnect){
					if(blueadapter==null){
						txt_tip.setText(getResources().getString(R.string.page_action_meimokuai));
					}else{
						if(blueadapter.enable()){
							txt_tip.setText(getResources().getString(R.string.page_action_chongqi));
						}
					}
				}
			}else if(msg.what==5557){
				invalidateUI();
			}else if(msg.what==5558){
//				Log.e("发送时间对比","蓝牙连接时间"+ODateTime.time2StringHHmmss(System.currentTimeMillis()));
				new CountDownTimerMy(2100, 650) {
					@Override
					public void onTick(long millisUntilFinished) {
						timeCount++;
//						Log.e("发送时间对比","间隔时间"+timeCount);
						if(timeCount==3){
//							Log.e("发送时间对比","靠近开时间"+ODateTime.time2StringHHmmss(System.currentTimeMillis()));
					...
					public void onFinish() {
						timeCount=0;
//						Log.e("发送时间对比","离开锁时间"+ODateTime.time2StringHHmmss(System.currentTimeMillis()));
	...
	public void invalidateUI()  {
		...
		showTip();
		iv_batterry.setVisibility(View.INVISIBLE);
		tv_batterry.setVisibility(View.INVISIBLE);
		tv_batterry_mode.setVisibility(View.INVISIBLE);
		if(EquipmentManager.isMiniP()&&car!=null&&car.isActive==1){
			DataCarStatus carStatus=ManagerCarList.getInstance().getCurrentCarStatus();
			if(carStatus!=null){
				if(carStatus.voltage<=5){
					tv_batterry.setTextColor(Color.parseColor("#FD0000"));
					iv_batterry.setVisibility(View.VISIBLE);
					tv_batterry.setVisibility(View.VISIBLE);
					tv_batterry_mode.setVisibility(View.VISIBLE);
					if(carStatus.isOnline==0&&!BlueLinkReceiver.getIsBlueConnOK()){
						tv_batterry.setText("--");
						iv_batterry.setImageResource(R.drawable.batter_40);
						iv_batterry.setVisibility(View.INVISIBLE);
						tv_batterry.setVisibility(View.INVISIBLE);
						tv_batterry_mode.setVisibility(View.INVISIBLE);
					}else{
						Log.e("电池电量",carStatus.levelBattery+"");
						if(carStatus.levelBattery>0&&carStatus.levelBattery<=100){
							tv_batterry.setText(carStatus.levelBattery+"%");
							if(carStatus.levelBattery>=0&&carStatus.levelBattery<=5){
								iv_batterry.setImageResource(R.drawable.batter_0_20);
							}else if(carStatus.levelBattery>5&&carStatus.levelBattery<=20){
								iv_batterry.setImageResource(R.drawable.batter_20);
							}else if(carStatus.levelBattery>20&&carStatus.levelBattery<=50){
								iv_batterry.setImageResource(R.drawable.batter_40);
							}else if(carStatus.levelBattery>50&&carStatus.levelBattery<=70){
								tv_batterry.setTextColor(Color.parseColor("#48ff00"));
								iv_batterry.setImageResource(R.drawable.batter_60);
							}else if(carStatus.levelBattery>70&&carStatus.levelBattery<=90){
								tv_batterry.setTextColor(Color.parseColor("#48ff00"));
								iv_batterry.setImageResource(R.drawable.batter_80);
							}else if(carStatus.levelBattery>90&&carStatus.levelBattery<=100){
								tv_batterry.setTextColor(Color.parseColor("#48ff00"));
								iv_batterry.setImageResource(R.drawable.batter_100);
							}
						}else{
//							if(carStatus.levelBattery==-10){
//							tv_batterry.setText("--");
//							iv_batterry.setImageResource(R.drawable.batter_40);
//							}else{
							iv_batterry.setVisibility(View.INVISIBLE);
		...
		if(BlueLinkReceiver.getIsBlueConnOK()){
			...
			if ( EquipmentManager.isMini()||(EquipmentManager.isMinJiaQiang()&&EquipmentManager.isMinNoMozu())) {
				if(EquipmentManager.isActive()){
					tiplayout.setVisibility(View.VISIBLE);
					txt_tip.setText(getResources().getString(R.string.control_tip_open_bluetooth));
				}
			}else{
				if(!NetManager.instance().isNetworkConnected()){
					...
					if(carInfo==null||carInfo.isActive==0){
						...
						if(dataCarStatus==null||dataCarStatus.isOnline==0&&dataCarStatus.carId==carInfo.ide){
							...
							tiplayout.setVisibility(View.VISIBLE);
							txt_tip.setText(getResources().getString(R.string.control_tip_car_online_false));
						}else{
							tiplayout.setVisibility(View.GONE);
						}
					}

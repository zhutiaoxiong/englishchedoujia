...
//    private final TextView txt_net_model;
    ...
    private TextView load_txt, txt_jump,txt_jump_hulue;
    private LinearLayout left_seek_layout, right_seek_layout;
    private ImageView nokey_btn;
...
//        txt_net_model = findViewById(R.id.txt_net_model);
        ...
        txt_jump = findViewById(R.id.txt_jump);
        txt_jump_hulue= findViewById(R.id.txt_jump_hulue);
        ...
        ODispatcher.addEventListener(OEventName.NOKEY_KAIGUAN_CHANGE, this);
    }
...
//                    + "&query=itemId=005007";//启动参数
            ...
            new ToastTxt(GlobalContext.getCurrentActivity(), null, false).withInfo("Alipay jump failed").quicklyShow();
            e.printStackTrace();
        }
    }

    public void initEvents() {
        nokey_btn.setOnClickListener(new OnClickListenerMy() {
            @Override
            public void onClickNoFast(View v) {
                if(!BlueLinkReceiver.getIsBlueConnOK()){
                    new ToastTxt(GlobalContext.getCurrentActivity(), null).withInfo(getResources().getString(R.string.page_apply_tip_blue_unconnect)).show();
                    return;
                }
                boolean isOpen= ManagerNokey.getInstance().getSwitchOpen();
                boolean isClose= ManagerNokey.getInstance().getSwitchClose();
                if(isOpen||isClose){
                    ClipPopNokey.getInstance().show(90,nokey_btn,getResources().getString(R.string.page_online_close_wugan),"closeNokey",ViewControlPanelOnline.this);
                }else{
                    ClipPopNokey.getInstance().show(0,nokey_btn,"","openNokey",ViewControlPanelOnline.this);
                }
            }
        });
        txt_jump.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                //mAutoStart mBatteryOptimization
                String comand = "mAutoStart";
                if (comand.equals("mAutoStart")) {
                    FcfrtAppBhUtils.toAutoStart(getContext());
                } else if (comand.equals("mBatteryOptimization")) {

                }
            }
        });
        txt_jump_hulue.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                AlertDialog.Builder builder = new AlertDialog.Builder(getContext());
                builder.setIcon(R.drawable.ic_launcher)
                        .setTitle(getResources().getString(R.string.page_set_hulie_title))
                        .setMessage(getResources().getString(R.string.page_online_dianchishuoming))
                        .setPositiveButton(getResources().getString(R.string.page_online_set_hulue), new DialogInterface.OnClickListener() {
                            @RequiresApi(api = Build.VERSION_CODES.M)
                            @SuppressLint("BatteryLife")
                            @Override
                            public void onClick(final DialogInterface dialog, final int which) {
                                //!AutoStartAppUtil.isIgnoringBatteryOptimizations(getContext())
                                if (!AutoStartAppUtil.isIgnoringBatteryOptimizations(getContext())) {
                                    try {
                                        Intent intent = null;
                                        ...
                                        if (intent != null) {
                                            if (intent.resolveActivity(getContext().getPackageManager()) == null) {
                                                ...
                                                if (intent.resolveActivity(getContext().getPackageManager()) == null) {
                                                    Toast.makeText(getContext(), getResources().getString(R.string.page_online_cannot_start), Toast.LENGTH_SHORT).show();
                                                } else {
...
// check that the Battery app exists on this device
                                    ...
                                    Toast.makeText(getContext(), getResources().getString(R.string.page_online_already_baimingdan), Toast.LENGTH_SHORT).show();
                                }
                            }
                        })
                        .show();
        ...
        iv_alibaba.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (!TextUtils.isEmpty(disPlay.url)) {
                    ...
                    bundle.putString(ActivityWebDisPla.TITLE_NAME,  getResources().getString(R.string.page_online_to_ali));
        ...
        seekBarLeftDoor.setOnThumbChangeListner(new ViewClipSeekBar.OnThumbChangeListner() {
            ...
            public void getThumbChange(int location) {
                if (EquipmentManager.isMini()) {
                    new ToastTxt(GlobalContext.getCurrentActivity(), null, false).withInfo( getResources().getString(R.string.page_control_no_support)).quicklyShow();
                    return;
                }
...
//                    ODispatcher.dispatchEvent(OEventName.GLOBAL_POP_TOAST, "指令已发送");
                    ...
                    Toast toast = Toast.makeText(getContext(),  getResources().getString(R.string.page_control_zhiling), Toast.LENGTH_SHORT);
        ...
        seekBarRightDoor.setOnThumbChangeListner(new ViewClipSeekBar.OnThumbChangeListner() {
            ...
            public void getThumbChange(int location) {
                ...
                if (car != null && currentStatus != null && car.isActive == 1) {
                    if (currentStatus.isTheft == 1) {
                        ODispatcher.dispatchEvent(OEventName.GLOBAL_POP_TOAST,  getResources().getString(R.string.page_control_wuxiao));
                    ...
                    new TimeDelayTask().runTask(10000, new TimeDelayTask.OnTimeEndListener() {
                        ...
                        public void onTimeEnd() {
                            ...
                            if (BuildConfig.DEBUG)
                                Log.e("查看发送时间", "获取车辆状态时间" + System.currentTimeMillis());
                            getCarStatus();
                        }
                    });
                }
        ...
        img_switch_air_conditioning.setOnClickListener(new OnClickListnerZhu() {
            ...
            public void onClickNoFast(View v) {
                ...
                if (ClipPopControlAirConditionNew.time == 0 || nowTime - ClipPopControlAirConditionNew.time > 7000L) {
                    ...
                    new ToastTxt(GlobalContext.getCurrentActivity(), null).withInfo(getResources().getString(R.string.page_online_pinfan_kongtiao)).show();
                }
            }
        });
...
//        });
        img_refresh.setOnClickListener(new OnClickListenerMy() {
            ...
            public void onClickNoFast(View v) {
                if (EquipmentManager.isMini()) {
                    new ToastTxt(GlobalContext.getCurrentActivity(), null).withInfo(getResources().getString(R.string.page_car_not_sopport)).show();
                    return;
                }
...
//        });
    ...
    public void receiveEvent(String eventName, Object paramObj) {
        if (eventName.equals(OEventName.PRESS_REFRESH_RESULTOK)) {
            ...
            if (resultOK) {
                ODispatcher.dispatchEvent(OEventName.GLOBAL_POP_TOAST, getResources().getString(R.string.page_online_refresh_success));
            } else {
                ODispatcher.dispatchEvent(OEventName.GLOBAL_POP_TOAST,  getResources().getString(R.string.page_online_refresh_fail));
            }
            ...
            handleHideProdilog();
        }else  if (eventName.equals(OEventName.NOKEY_KAIGUAN_CHANGE)){
            handleSetNokeyStatus();
        }
    }

    private void handleSetNokeyStatus() {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case 11111:
                    ...
                    if (arg == 1) {
                        load_txt.setText(getResources().getString(R.string.page_online_zhengzai_shengchuang));
                    } else if (arg == 2) {
                        load_txt.setText(getResources().getString(R.string.page_online_zhengzai_jiangchaung));
                    } else if (arg == 3) {
                        load_txt.setText(getResources().getString(R.string.page_online_zhengzai_houweixiang));
                    }
                    break;
                case 11112:
                ...
                case 18660:
                    boolean isOpen= ManagerNokey.getInstance().getSwitchOpen();
                    boolean isClose= ManagerNokey.getInstance().getSwitchClose();
//                    Log.e("无钥匙","isOpen"+isOpen+"");
//                    Log.e("无钥匙","isClose"+isClose+"");
                    if(isOpen||isClose){
                        nokey_btn.setSelected(true);
                    }else{
                        nokey_btn.setSelected(false);
                    }
                    break;
    ...
    private void setMeterUI(DataCarStatus custatus) {
        if (custatus == null || custatus.miles == 0) {
            ...
            txt_meter.setText(String.valueOf(custatus.miles));
        }
    }
...
//            setTempoBorrowCarUI(null);
            nokey_btn.setSelected(false);
            if (img_switch_air_conditioning.getVisibility() != INVISIBLE)

        ...
        if(key.equals("operate")){
            ...
            if(opp.equals(getResources().getString(R.string.stop_the_authorization))){
                new ToastConfirmNormal(GlobalContext.getCurrentActivity(), null,false)
                        .withTitle(ResourceUtil.getResourceStr(getContext(),R.string.page_borrow_cancle_fuchezhu))
                        .withInfo(ResourceUtil.getResourceStr(getContext(),R.string.page_borrow_sure_cancle)+selectedUser.carinfo.num+" "+selectedUser.userinfo.name+" "+selectedUser.userinfo.phoneNum+ResourceUtil.getResourceStr(getContext(),R.string.page_borrow_auhor_ma))

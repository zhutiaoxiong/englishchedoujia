        ...
        if (needUser) {
            if(selectUsaer!=null&&!TextUtils.isEmpty(selectUsaer.phoneNum)){
                new ToastConfirmNormal(GlobalContext.getCurrentActivity(), null,false)
                        .withTitle(ResourceUtil.getResourceStr(getContext(),R.string.page_user_liking_shouquan_fuzhu))
                        .withInfo(ResourceUtil.getResourceStr(getContext(),R.string.page_borrow_you_sure)+txt_pickcar.getText().toString()+ResourceUtil.getResourceStr(getContext(),R.string.give)+selectUsaer.phoneNum+ResourceUtil.getResourceStr(getContext(),R.string.what))

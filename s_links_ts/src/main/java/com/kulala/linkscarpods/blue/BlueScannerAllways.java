...
//        isScanning=false;
    ...
    private  ScanCallback mLeScanCallback = new ScanCallback() {
        ...
        public void onScanResult(int callbackType, ScanResult result) {
            ...
            if(record.getDeviceName()!=null&&!record.getDeviceName().equals("")){
...
//                    String devicename=parceName(bytes,31);
                    device1.name=devicename;
                    if (BuildConfig.DEBUG)  Log.e("blue","scanLeDevice Func:扫到设备设备名称asdsadasdad:"+ devicename);
                }
            }
...
//            }
        ...
        private String parceName(byte[] bytes,int startLocation){
            if (BuildConfig.DEBUG) Log.e("blue","得到蓝牙名称---:");
            String name = "";
            int pos=startLocation;
            int len=(int)bytes[pos]&0xff;
            int datatype;
            byte[]data;
            while (len>0){
                datatype=(int)bytes[++pos]&0xff;
                data=Arrays.copyOfRange(bytes,pos,pos+len-1);
                switch (datatype){
                  case  0x08:
                  case 0x09:
                      name=new String(data);
                      if (BuildConfig.DEBUG) Log.e("blue","得到蓝牙名称:"+name);
                    break;
                }
                pos += len - 1;
                len = (int)bytes[pos]&0xff;
            }
            return name;
        }

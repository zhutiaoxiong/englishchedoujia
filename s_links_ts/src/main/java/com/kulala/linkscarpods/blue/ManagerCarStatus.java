...
public class ManagerCarStatus {
    ...
    public static void setData0x21(byte[] data){
        if(data.length!=8)return;
        DataCarStatus newStatus =saveCarStaus( data);
        DataCarStatus status=ManagerCurrentCarStatus.getInstance().getCarStatus();
         if (BuildConfig.DEBUG) Log.e("——————", "後臺保存的固定status "+status );
         if (BuildConfig.DEBUG) Log.e("——————", "後臺蓝牙获取的新的 "+newStatus );
...
//                LcdManagerCarStatus.sendCarStatus(newStatus);
               ...
               BlueLinkControlLcdKey.getInstance().sendMessage(ByteHelper.bytesToHexString(LcdManagerCarStatus.ObjectToByteBlue(newStatus)),true);
            }
        }
...
//        carInfo.startunpre = normals[3];
        ...
        ManagerCurrentCarStatus.getInstance().setCarStatus(status);
        pre21 = data;
    }
...
//        carInfo.startunpre = normals[3];
    ...
    //fin
    public static void setData0x22(byte[] data){
        if(data.length!=8)return;
        DataCarStatus status =  ManagerCurrentCarStatus.getInstance().getCarStatus();
        if(status == null)return;
...
//        }
        ...
        byte remainOilll=data[6];
        byte remainOi2=data[7];
        status.remainOil=(remainOi2*256+remainOilll)/10;
        status.voltage = noSign(data[3])/10;
        Log.e("电池电压ssds",data[3]+"");
        ManagerCurrentCarStatus.getInstance().setCarStatus(status);
        BlueLinkControlLcdKey.getInstance().sendMessage(ByteHelper.bytesToHexString(LcdManagerCarStatus.ObjectToByteBlue(status)),true);
